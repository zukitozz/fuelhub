...
export const HistoricTable = ({ fechaEmision, page, listPages, handleAddPage }: HistoricProps) => {
    ...
    useEffect(() => {
        if (paginate) {
            ...
            if (exists === -1) {
                handleAddPage({ lastValue: paginate, page: listPages.filter((pageItem) => {
                                    ...
                                    }).reduce((max, p) => (p.page > max ? p.page : max), 0) + 1 });
            }
        }
    }, [listPages, paginate, handleAddPage, fechaEmision]);
    useEffect(() => {
        mutate();
    }, [fechaEmision, page, mutate]);
